...
imgsetfile = os.path.join(args.SIXray_root, 'Testset', 'test.txt')
...
def do_python_eval(use_07=False):
    ...
    for i, cls in enumerate(labelmap):
        ...
        rec, prec, ap = voc_eval(
            filename, annopath, imgpath, imgsetfile, cls,
            ovthresh=0.5, use_07_metric=use_07_metric)
        aps += [ap]
        # print('AP for {} = {:.4f}'.format(cls, ap))
        # with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
        #     pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
    print("core_AP, {:.4f}".format(aps[0]))
    print("coreless_AP, {:.4f}".format(aps[1]))
...
def voc_eval(detpath, annopath, imgpath, imagesetfile, classname, ovthresh=0.5, use_07_metric=True):
